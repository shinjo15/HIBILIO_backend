...
final readonly class IssuePersistentLogin implements IssuePersistentLoginInterface
{
    ...
    public function execute(IssuePersistentLoginInputPort $input): IssuePersistentLoginOutputPort
    {
        ...
        $expiresAt = new PersistentLoginExpiresAt($this->clock->now()->modify('+30 days'));
        $this->tokens->save(new PersistentLoginToken($selector, $input->accountIdentifier(), $this->hashService->hash($validator), $expiresAt));
        $this->authService->login($input->accountIdentifier());
        $this->cookie->queue($selector->value().'.'.$validator, self::MINUTES);
