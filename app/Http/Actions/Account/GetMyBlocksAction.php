...
final readonly class GetMyBlocksAction
{
    ...
    public function __invoke(GetMyBlocksRequest $request): JsonResponse
    {
        ...
        return new JsonResponse(['blocks' => array_map(static fn (array $account): array => ['account_identifier' => $account['accountIdentifier'], 'account_name' => $account['accountName'], 'account_bio' => $account['accountBio'], 'icon_image_url' => $account['iconImageUrl']], $output->blocks())]);
    }
}
